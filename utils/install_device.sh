...
sudo systemd-nspawn -D $ROOTFS bash -c "export DEBIAN_FRONTEND=noninteractive && apt-get update && apt-get -y full-upgrade"

if [ ! -z $EXTRAPKGS ]; then
  sudo systemd-nspawn -D $ROOTFS bash -c "export DEBIAN_FRONTEND=noninteractive && apt-get -y install ${EXTRAPKGS//,/ }"
fi
